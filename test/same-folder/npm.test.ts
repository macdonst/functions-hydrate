    ...
    describe('invoke npm folder', () => {
        it('should run local test typescript source', async () => {
            const resource = {
                ...
                src: './same-folder/npm/functions/typescript',
            }
            ...
            expect(plainLogs).toContain('same-folder-npm-typescript')
        ...
        it('should run local test javascript source', async () => {
            const resource = {
                name: 'javascript',
                type: 'sanity.function.document-publish',
                src: './same-folder/npm/functions/javascript',
            }
            const response = await invoke(resource, {})
            ...
            expect(plainLogs).toBeTruthy()
            expect(plainLogs).toContain('same-folder-npm-javascript')
