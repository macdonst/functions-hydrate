    ...
    describe('invoke pnpm folder', () => {
        it('should run local test typescript source', async () => {
            const resource = {
                ...
                src: './same-folder/pnpm/functions/typescript',
            }
            ...
            expect(plainLogs).toContain('same-folder-pnpm-typescript')
        ...
        it('should run local test javascript source', async () => {
            const resource = {
                name: 'javascript',
                type: 'sanity.function.document-publish',
                src: './same-folder/pnpm/functions/javascript',
            }
            const response = await invoke(resource, {})
            ...
            expect(plainLogs).toBeTruthy()
            expect(plainLogs).toContain('same-folder-pnpm-javascript')
