    ...
    describe('invoke yarn folder', () => {
        it('should run local test typescript source', async () => {
            const resource = {
                ...
                src: './functions-folder/yarn/functions/typescript',
            }
            ...
            expect(plainLogs).toContain('functions-folder-yarn-typescript')
        ...
        it('should run local test javascript source', async () => {
            const resource = {
                name: 'javascript',
                type: 'sanity.function.document-publish',
                src: './functions-folder/yarn/functions/javascript',
            }
            const response = await invoke(resource, {})
            ...
            expect(plainLogs).toBeTruthy()
            expect(plainLogs).toContain('functions-folder-yarn-javascript')
