    ...
    describe('invoke npm folder', () => {
        it('should run local test typescript source', async () => {
            const resource = {
                ...
                src: './functions-folder/npm/functions/typescript',
            }
            ...
            expect(plainLogs).toContain('functions-folder-npm-typescript')
        ...
        it('should run local test javascript source', async () => {
            const resource = {
                name: 'javascript',
                type: 'sanity.function.document-publish',
                src: './functions-folder/npm/functions/javascript',
            }
            const response = await invoke(resource, {})
            ...
            expect(plainLogs).toBeTruthy()
            expect(plainLogs).toContain('functions-folder-npm-javascript')
