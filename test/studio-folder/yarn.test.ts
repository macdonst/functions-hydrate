...
describe('invoke studio folder', () => {
    describe('invoke yarn folder', () => {
        it('should run local test typescript source', async () => {
            const resource = {
                name: 'typescript',
                type: 'sanity.function.document-publish',
                src: './studio-folder/yarn/functions/typescript',
            }
            ...
            expect(plainLogs).toContain('studio-folder-yarn-typescript')
        ...
        it('should run local test javascript source', async () => {
            const resource = {
                name: 'javascript',
                type: 'sanity.function.document-publish',
                src: './studio-folder/yarn/functions/javascript',
            }
            const response = await invoke(resource, {})
            ...
            expect(plainLogs).toBeTruthy()
            expect(plainLogs).toContain('studio-folder-yarn-javascript')
